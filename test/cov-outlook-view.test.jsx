...
describe('OutlookView (enumeration result rendering)', () => {
  ...
  it('states a one-point, positive-GD threshold in the singular and its rival groups in the plural', () => {
    // A survivor sitting on exactly ONE point with a POSITIVE goal difference,
    // who needs results in TWO other groups. Every number in that sentence takes
    // the opposite form to the ordinary case — "1 pt" not "1 pts", "+2" not "2",
    // "rival thirds" not "rival third".
    render(<OutlookView matches={COMPLETE} />)
    send({
      type: 'done',
      result: allLocked(),
      ...
      requirements: {
        Ghana: {
          ownGroupComplete: true,
          variable: [
            { group: 'C', contenders: ['Brazil', 'Scotland'] },
            { group: 'E', contenders: ['Belgium', 'Egypt'] },
          ],
          needAtLeast: 2,
          profile: { Pts: 1, GD: 2 },
        },
